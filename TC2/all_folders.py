...
def show():
    st.markdown("<h2>All Folders (Auto from Teams)</h2>", unsafe_allow_html=True)
    st.markdown("<h3>Click folder to open in SharePoint / Teams</h3>", unsafe_allow_html=True)

    search = st.text_input("🔍 Search Samples", placeholder="Search sample name...", key="search_tc2_folders")
    ...
    try:
        ...
        df = pd.read_excel(excel_path, sheet_name="TC2-Raw data")
