...
def show():
    ...
    sheet_name = "TC1-Raw data & Report"
    ...
    def format_multiline_links(names, links):
        """将多个名称和对应的链接格式化为HTML，每个报告前都有📄符号"""
        if not names:
            return ""
        html_lines = []
    ...
    def parse_time_to_date(time_value):
        ...
        # 尝试解析包含时区的时间格式: 2025/4/21 23:56:34 -08:00
        import re

        # 匹配 YYYY/M/D 或 YYYY-MM-DD
        date_patterns = [
            r'(\d{4})/(\d{1,2})/(\d{1,2})',  # 2025/4/21
            r'(\d{4})-(\d{1,2})-(\d{1,2})',  # 2025-4-21
        ]
    ...
    try:
        ...
        # Sample日期范围筛选 - 现在使用 YYYY-MM-DD 格式，字符串比较等同于日期比较
        ...
        if len(filtered_rows) > 0:
            ...
            for row in filtered_rows:
                ...
                if raw_date_display and raw_date_display.strip() and raw_date_display != 'nan':
                    # 显示时将 YYYY-MM-DD 转为 YYYY/MM/DD 便于阅读
