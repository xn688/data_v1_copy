...
def show():
    ...
    try:
        ...
        if 'tc3_selected_projects' not in st.session_state:
            st.session_state.tc3_selected_projects = all_projects
        if 'tc3_selected_voltages' not in st.session_state:
            st.session_state.tc3_selected_voltages = all_voltages

        col_filter1, col_filter2 = st.columns(2)

        with col_filter1:
            if st.session_state.tc3_selected_voltages:
                available_projects = df[df['Voltage Condition'].isin(st.session_state.tc3_selected_voltages)][
            ...
            selected_projects = st.multiselect(
                ...
                default=[p for p in st.session_state.tc3_selected_projects if p in available_projects],
        ...
        with col_filter2:
            if st.session_state.tc3_selected_projects:
                available_voltages = df[df['Project Name'].isin(st.session_state.tc3_selected_projects)][
            ...
            selected_voltages = st.multiselect(
                ...
                default=[v for v in st.session_state.tc3_selected_voltages if v in available_voltages],
                placeholder="Select voltage conditions...",
                key="tc3_voltage_multiselect"
            )
            st.session_state.tc3_selected_voltages = selected_voltages
