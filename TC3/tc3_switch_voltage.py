...
def show():
    ...
    try:
        ...
        if 'tc3_temp_projects' not in st.session_state:
            st.session_state.tc3_temp_projects = all_projects.copy()
        if 'tc3_temp_voltages' not in st.session_state:
            st.session_state.tc3_temp_voltages = all_voltages.copy()
        ...
        with col_filter1:
            # 根据已选的电压条件，计算可用的项目选项
            current_voltages = st.session_state.tc3_temp_voltages
            if current_voltages:
            ...
            # 获取当前选中的项目，并过滤掉无效值
            current_projects = st.session_state.tc3_temp_projects
            valid_projects = [p for p in current_projects if p in available_projects]
            if valid_projects != current_projects:
                st.session_state.tc3_temp_projects = valid_projects

            selected_projects = st.multiselect(
                "Filter by Project Name",
                options=available_projects,
                default=st.session_state.tc3_temp_projects,
                placeholder="Select projects...",
                key="tc3_project_multiselect"
            )
            st.session_state.tc3_temp_projects = selected_projects
        ...
        with col_filter2:
            ...
            if valid_voltages != current_voltages:
                st.session_state.tc3_temp_voltages = valid_voltages

            selected_voltages = st.multiselect(
                "Filter by Voltage Condition",
                options=available_voltages,
                default=st.session_state.tc3_temp_voltages,
                placeholder="Select voltage conditions...",
                key="tc3_voltage_multiselect"
            )
            st.session_state.tc3_temp_voltages = selected_voltages

        # ========== 应用筛选 ==========
        filtered_df = df.copy()

        if st.session_state.tc3_temp_projects:
            filtered_df = filtered_df[filtered_df['Project Name'].isin(st.session_state.tc3_temp_projects)]
        if st.session_state.tc3_temp_voltages:
            filtered_df = filtered_df[filtered_df['Voltage Condition'].isin(st.session_state.tc3_temp_voltages)]
        ...
        # 创建一个筛选状态的唯一标识
        filter_key = f"{len(st.session_state.tc3_temp_projects)}_{len(st.session_state.tc3_temp_voltages)}"
